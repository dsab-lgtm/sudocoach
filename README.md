...
UI screenshots are a local developer check and are never committed. Run
`npm run test:e2e:visual` to compare the local snapshots, or
`npm run test:e2e:visual:update` after intentionally changing the UI. GitHub Actions
still runs the complete non-image E2E suite, including responsive, keyboard, dialog,
